      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto flex-1 bg-white space-y-4">
          <div className="bg-teal-50 text-teal-800 p-4 rounded-2xl">
            ...
            <strong className="text-2xl font-extrabold">Rp {getCartTotal()}</strong>
          </div>
              ...
              {Object.values(cart).map((item) => (
                <div key={item.obat.id} className="p-3.5 flex justify-between text-sm">
                  <div>
                    ...
                    <p className="text-xs text-zinc-400">{item.jumlah} unit x Rp {item.obat.harga}</p>
                  </div>
                  <span className="font-bold text-zinc-950">Rp {item.obat.harga * item.jumlah}</span>
                </div>
              ))}
            </div>
          </div>
